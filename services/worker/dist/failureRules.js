...
exports.RETRY_BASE_DELAY_MS = exports.MAX_ATTEMPTS = exports.RATE_LIMIT_DELAY_MS = void 0;
exports.classifyFailure = classifyFailure;
exports.decideFailureAction = decideFailureAction;
exports.latestJobsPerPlatform = latestJobsPerPlatform;
exports.derivePostStatus = derivePostStatus;
 ...
 */
function decideFailureAction(input) {
    const kind = classifyFailure(input.httpStatus, input.message);
    if (kind === 'retryable' && input.attempt < input.maxAttempts)
        return { action: 'retry', kind };
 ...
 */
function derivePostStatus(jobs) {
    const latest = latestJobsPerPlatform(jobs);
    if (latest.length === 0)
        return null;
    if (latest.some((j) => j.status === 'scheduled' || j.status === 'processing'))
        return null;
    if (latest.every((j) => j.status === 'completed'))
        return 'published';
    if (latest.some((j) => j.status === 'failed'))
        return 'failed';
    return null;
}
exports.RATE_LIMIT_DELAY_MS = 60 * 60 * 1000;
...
exports.RETRY_BASE_DELAY_MS = 60_000;
/** Delay before the retry that follows the `failures`-th failed attempt (1-based). */
function retryDelayMs(failures, baseMs = exports.RETRY_BASE_DELAY_MS) {
...
function failureMessage(excerpt, platform, message, attempts) {
