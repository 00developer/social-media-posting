...
exports.THREADS_MAX_BYTES = exports.MAX_CONTENT_LENGTH = void 0;
exports.latestJobsPerPlatform = latestJobsPerPlatform;
exports.getEditBlockReason = getEditBlockReason;
exports.getContentProblem = getContentProblem;
exports.MAX_CONTENT_LENGTH = 10_000;
/** Same limit the Threads adapter enforces at publish time (UTF-8 bytes). */
exports.THREADS_MAX_BYTES = 500;
const timeOf = (iso) => {
...
function getEditBlockReason(jobs) {
    ...
    if (processing > 0)
        return 'This post is being published right now, so its caption can no longer be edited.';
    if (completed > 0 && completed === latest.length)
        return 'This post is already published, so its caption can no longer be edited.';
    if (completed > 0)
        return 'This post is already published on some platforms, so its caption can no longer be edited.';
    return null;
}
/** null = the caption is acceptable for the platforms the post targets; otherwise the reason it isn't. */
function getContentProblem(content, jobs) {
    if (typeof content !== 'string')
        return 'content must be a string';
    if (content.trim().length === 0)
        return 'content cannot be empty';
    if (content.length > exports.MAX_CONTENT_LENGTH)
        return `content cannot be longer than ${exports.MAX_CONTENT_LENGTH} characters`;
    const targetsThreads = latestJobsPerPlatform(jobs).some((j) => j.platform === 'threads');
    if (targetsThreads && Buffer.byteLength(content, 'utf8') > exports.THREADS_MAX_BYTES) {
        return `Threads text exceeds the ${exports.THREADS_MAX_BYTES} UTF-8 bytes limit.`;
    }
    return null;
}
